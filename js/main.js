...
// Задание 1
// Получить массив имен всех пoльзователей (поле name).
...
const calculateTotalBalance = users => {
  return users.reduce((amount, user) => {
    amount += user.balance;
    return amount;
  }, 0);
};
...
// должно быть повторяющихся умений и они должны быть отсортированы
// в алфавитном порядке.

const getSortedUniqueSkills = users => {
  return users
    .reduce((skillsUsers, user) => {
      const skillsNotReply = user.skills.filter(
        skill => !skillsUsers.includes(skill),
      );
      skillsUsers.push(...skillsNotReply);
      return skillsUsers;
    }, [])
    .sort();
};
